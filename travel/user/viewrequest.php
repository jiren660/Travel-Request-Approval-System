...
<?php if (!empty($request['handled_by'])): ?>
  <?php
    // fetch admin name (handled_by)
    $stmt = $conn->prepare("SELECT first_name, last_name FROM users WHERE id = ?");
    $stmt->execute([$request['handled_by']]);
    $handler = $stmt->fetch(PDO::FETCH_ASSOC);
  ?>
  <p><strong>Handled By:</strong> 
     <?= htmlspecialchars($handler['first_name'] . ' ' . $handler['last_name']) ?>
  </p>
<?php endif; ?>

<?php if (!empty($request['handled_at'])): ?>
  <p><strong>Handled At:</strong> 
     <?= date('F d, Y h:i A', strtotime($request['handled_at'])) ?>
  </p>
<?php endif; ?>
