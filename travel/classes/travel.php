...
class Travel {
    ...
    public $handled_by = null;
    public $handled_at = null;
...
public function viewAllRequests() {
    ...
    $sql = "SELECT 
                ...
                h.first_name AS handled_first_name, 
                h.last_name AS handled_last_name
            FROM travel_request tr
            JOIN users u ON tr.user_id = u.id
            LEFT JOIN users h ON tr.handled_by = h.id
            ORDER BY 
                CASE tr.status 
                    WHEN 'Pending' THEN 1
...
// APPROVE REQUEST
public function approveRequest($request_id, $approved_budget = null, $remarks = null, $handled_by = null) {
    $conn = $this->db->connect();

    // Ensure valid admin ID
    if (empty($handled_by)) {
        throw new Exception("Missing handler ID for approval action.");
    }
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bindValue(":approved_budget", $approved_budget !== null ? $approved_budget : null, $approved_budget !== null ? PDO::PARAM_STR : PDO::PARAM_NULL);
    $stmt->bindValue(":remarks", $remarks !== null ? $remarks : null, $remarks !== null ? PDO::PARAM_STR : PDO::PARAM_NULL);
    $stmt->bindParam(":handled_by", $handled_by, PDO::PARAM_INT);
    $stmt->bindParam(":request_id", $request_id, PDO::PARAM_INT);
...
// REJECT REQUEST
public function rejectRequest($request_id, $remarks = null, $handled_by = null) {
    $conn = $this->db->connect();

    // Ensure valid admin ID
    if (empty($handled_by)) {
        throw new Exception("Missing handler ID for rejection action.");
    }
